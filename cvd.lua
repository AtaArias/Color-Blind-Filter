...
local shader_code = [[
    extern mat3x3 Matriz;

    vec4 effect(vec4 color, Image image, vec2 uv, vec2 sc){
        vec4 pixel = vec4(Texel(image, uv));
        return vec4(Matriz * pixel.rgb, pixel.a) * color;
    }
]]

local function color_filter(kind, severness)
    local filter = love.graphics.newShader(shader_code)

    if severness < 0 or severness > 10 then
        print("severnes must be from 1 to 10")
        return
    elseif (kind ~= 't') and (kind ~= 'd') and (kind ~= 'p') then
        print("kind isn't t nor d nor p")
        return
    end

    love.graphics.setShader(filter)
    filter:send('Matriz', corrections[kind][severness])
end

cvd.start = color_filter
cvd.finish = function() love.graphics.setShader() end
